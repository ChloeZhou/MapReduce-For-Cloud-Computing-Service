...
//  ServerC
//
//  Created by Keying Zhou on 11/12/16.
...
int main() {
        ...
        printf("The Server C is up and running using UDP on port 23872.\n");
while(1)
{
    //creat socket
    ...
    bind(servc_sock, (struct sockaddr*)&servc_addr, sizeof(servc_addr));
    ...
    recvfrom(servc_sock, recvfromaws, sizeof(recvfromaws), 0, (struct sockaddr *)&aws_addr, &aws_addr_size);
    ...
    printf("Server C has receive %d numbers.\n",nums);
   ...
   // printf("%d\n%d\n",recv[0],recv[99]);    //for test
    ...
    printf("The Server C has successfully finished the reduction %s: %lld\n",functionname,result);
    ...
    char ACK;
    ACK='\0';
    recvfrom(servc_sock, &ACK, sizeof(ACK), 0, (struct sockaddr *)&aws_addr, &aws_addr_size);
    close(servc_sock);
    ...
    printf("The Server C has successfully finished sending the reduciton value to AWS server.\n");
    close(aws_sock);
}
    return 0;
}
