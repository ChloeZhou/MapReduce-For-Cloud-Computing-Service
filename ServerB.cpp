...
int main(int argc, const char * argv[]) {
    ...
    printf("The Server B is up and running using UDP on port 22872.\n");
    ...
    while(1)
    {
        ...
        bind(servb_sock, (struct sockaddr*)&servb_addr, sizeof(servb_addr));
        ...
        recvfrom(servb_sock, recvfromaws, sizeof(recvfromaws), 0, (struct sockaddr *)&aws_addr, &aws_addr_size);
        ...
        printf("Server B has receive %d numbers.\n",nums);
        ...
        printf("The Server B has successfully finished the reduction %s: %lld\n",functionname,result);
        ...
        char ACK;
        ACK='\0';
        recvfrom(servb_sock, &ACK, sizeof(ACK), 0, (struct sockaddr *)&aws_addr, &aws_addr_size);
        close(servb_sock);
        ...
        printf("The Server B has successfully finished sending the reduciton value to AWS server.\n");
        close(aws_sock);
    }
    return 0;
}
